...
- 実測で確認済みの結論（詳細は`docs/RECENT_WORK.md`参照）：
  ...
  - **上記を踏まえた研究ストーリーの組み替え（推奨）**：(1) 提案手法はContrastive LIME（真の係数復元・極端領域で有意優位）。(2) 一貫性は「共有支持集合＋Contrastive再フィット」という選択肢として提示し、その利得（fidelity・安定性・overlap=1）と代償（ペア固有特徴の取りこぼし）を実測で示す。(3) Fisherは「係数推定器としては$S_W$尺度の偏りで回帰に本質的に勝てず、共有構造の供給源としてもRidge集約と差がない」ことを機構レベルで示した分析章として位置づける。Fisherだけが原理的に有利になりうる未検証の状況は、黒箱が確率を返さずラベルしか返さない場合（OVR/Contrastiveはlog-oddsを回帰できない）。
