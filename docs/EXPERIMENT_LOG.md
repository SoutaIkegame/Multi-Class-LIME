...
- OVR-unionの複雑さがKを超える非対称性を解消した「複雑さを正確に揃えた」比較（例：各クラスK/2個ずつ選び和集合をK近辺に抑える）は未実装。

---

## 13. 現時点の採用方針（2026-09-05時点、フェーズ12の結果を踏まえて要更新）

**注記（2026-09-06）**：以下はフェーズ11.5までの結果に基づく。フェーズ12（疎な設定でのOVO vs OVR比較）は「OVOが少ない特徴で明確に勝つ」という主張を支持しておらず、特に提案Cについては疎な設定で逆転している。中心的な問いへの回答は現時点で「条件依存・両義的」であり、以下の方針は暫定的なものとして読むこと。
...
6. **フェーズ12により追加された留保**：疎な（表示特徴数を絞った）設定では、ContrastiveはOVR-unionとほぼ同点（複雑さで不利な条件でも）、提案CはOVR-union・Contrastiveの両方にわずかに劣る。密な設定（フェーズ11.5）とは逆の序列になっており、**「Cを主軸とする」（2番目）という方針は密な全特徴フィットの場合に限定される**。修論の記述では「密/疎のどちらの設定を主張の中心に置くか」を明示する必要がある。
...
- **OVR-unionの複雑さがKを超える非対称性を解消した比較は未実施**（各クラスK/2個ずつ選ぶなど）。
